...
Recorder.deregister = function () {
    ...
    InnoVisionITS.deHighlightPreviousElement();
...
function recordClickEvent(event) {
    if (event.which === 1) {
        ...
        if (findClickableElement(clickElement)) {
            InnoVisionITS.sendRecordedObject(clickElement, "Click");
        }
...
function unorderedListClickListener(clickElement) {
    ...
    if (Util.isNotNullOrEmpty(value))
        InnoVisionITS.sendRecordedObject(clickElement.parentElement, "selectValueFromUnorderedList", "@" + value);
    else
        InnoVisionITS.sendRecordedObject(clickElement.parentElement, "selectIndexFromUnorderedList", "@" + getElementCount(clickElement));
}
    ...
    if (element.tagName.toLowerCase() === "select") {
        ...
        if (Util.isNotNullOrEmpty(input)) {
            if (element.type.toLowerCase() === "password")
                InnoVisionITS.sendRecordedObject(element, "setEncrypted", "@" + input);
            else
                InnoVisionITS.sendRecordedObject(element, "Set", "@" + input);
        }
        else
            InnoVisionITS.sendRecordedObject(element, "clear");
    }
}

function recordSelectEvent(element) {
    var input = "@" + Util.removeWhiteSpace(element.selectedOptions[0].text);
    InnoVisionITS.sendRecordedObject(element, "selectByVisibleText", input);
}
...
function checkAndRecordContextEvent(data) {
    if (isElementActionWithInput(data)) {
        if (context_element)
            InnoVisionITS.sendRecordedObject(context_element, data.command, getInput(data));
    } else if (isElementAction(data)) {
        if (context_element)
            InnoVisionITS.sendRecordedObject(context_element, data.command);
    } else if (isBrowserActionWithInput(data)) {
        InnoVisionITS.sendRecordedObject(null, data.command, getInput(data));
    } else if (isBrowserAction(data)) {
        InnoVisionITS.sendRecordedObject(null, data.command);
    }
...
var checkForMutatation = function (target) {
    var observer = new MutationObserver(function (mutations) {
        ...
        InnoVisionITS.sendRecordedObject(mutations[0].target, "Click");
        observer.disconnect();
    });
