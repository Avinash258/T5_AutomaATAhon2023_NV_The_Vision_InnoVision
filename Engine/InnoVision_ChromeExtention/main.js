...
function InnoVisionITS() {
}

InnoVisionITS.deHighlightPreviousElement = function () {
    Util.deghlightElement(previousElement);
};

InnoVisionITS.sendSpiedObject = function (element, flag) {
    var data = {};
    data.type = "Object";
...
InnoVisionITS.sendHealResult = function (data) {
    data.action = "heal";
    sendValues(JSON.stringify(data));
};

InnoVisionITS.sendRecordedObject = function (element, method, input) {
    var data = {};
    data.action = "record";
    data.method = method;
    data.input = input;
...
InnoVisionITS.getFrameValue = function () {
    return getFrameData();
};
